...
And another example using the `Hillshade` layer from the "dhmv": digital elevation model Flanders:
...
To know which layers are available in a given WCS service, you can use the following code:

```{r}
get_wcs_layers(wcs = "dhmv") |>
  knitr::kable()
```
