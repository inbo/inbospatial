...
This result is than saved temporarily to a `.GML` file and read with `sf::read_sf()`.
...
This time, both methods yield the same result apart from a different naming of the CRS (but their `WKT` specification is the same - so it's the same CRS).
...
The only result difference is again the naming of the CRS.
...
kmi_synop <- get_feature_wfs(
  ...
  property_name = paste("code", "timestamp", "temp_max",
                        "the_geom", sep = ","),
  cql_filter = "((code = 6447) AND (temp_max IS NOT NULL) AND (timestamp >= '2000-01-01 00:00:00.000'))")
...
This can also be done using the `wkt_filter` argument from `sf::read_sf`, but we do not show it here.
...
soil_type_sf <- read_sf(
  ...
  wkt_filter = sprintf("ST_Intersects(geom,POINT(%s %s))",
                  x_lam, y_lam))
