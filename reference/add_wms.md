...
The `add_wms_*()` functions are shorthand alternatives for a fully
specified
...
- add_wms_legend:
  ...
  Logical. Is a legend to be added for this WMS? Note that the legend
  cannot be toggled on and off in the map; it is plotted as a separate,
  static legend.
...
- `_forestnature`:
  ...
  Public forest and nature areas managed by the Flemish Nature & Forest
  Agency
